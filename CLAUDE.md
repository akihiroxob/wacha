...
npm run build         # frontend typecheck (tsc -p src/frontend) + vite build into public/
...
- `src/presentation/controller/PageController.ts` — JSON API controller for the web UI (`/api/*` routes), calling the same use cases as the MCP tools. Response/request shapes are typed against `src/shared/apiTypes.ts`.
...
- `src/frontend/` — React 19 + Vite + TanStack Query + Tailwind v4 SPA. Single npm package: all deps live in the root package.json; `src/frontend/` holds the SPA source, `vite.config.ts` (with `root` set to its own dir), and its own tsconfig. `vite build` emits into `public/`, which Hono serves with an `index.html` fallback for unknown GET paths. Dev: Vite on :5173 proxies `/api` to Hono. Data flow: one aggregate query per project (`useProject`, 5s polling — agents mutate state via MCP at any time) and mutations that invalidate it (`src/frontend/lib/queries.ts`).
- `src/shared/apiTypes.ts` — self-contained DTO/wire types shared by server (`@shared/*` alias) and frontend. Keep it free of imports from backend or frontend modules.

Path aliases (see `tsconfig.json`): `@container`, `@application/*`, `@constants/*`, `@mcp/*`, `@domain/*`, `@repository/*` (→ `infrastructure/repository`), `@database/*` (→ `infrastructure/database`), `@bootstrap/*`, `@controller/*`, `@shared/*` (→ `src/shared`). Imports use explicit `.ts` extensions (NodeNext module resolution). The frontend has its own tsconfig (`moduleResolution: bundler`, aliases `@/*` and `@shared/*`).
