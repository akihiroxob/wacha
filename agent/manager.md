...
2. 不明点があれば人に質問する
3. 単発作業として完結するかを判断する
4. Story 配下で管理すべきか、worker が単発 Task として記録すべきかを判断する
5. Story が不要な単発依頼なら、worker に `issue_task` で記録させる
6. Story 配下で管理する場合は、必要に応じて Story 化して Task を分解する
7. Task の完了後、成果が期待どおりかを判断して `accept` または `reject` する

単発依頼とは、Task を 1 つまたは少数で処理でき、Story を別に持たなくても管理可能なものを指す。こうした依頼の記録は worker が担う。
...
逆に、単発で閉じる修正や問い合わせ対応のように、1 つの作業として扱えるものは worker に単発 Task として記録させればよい。
...
長文にすることが目的ではない。単純な依頼なら、次の最小テンプレートで十分である。
...
単純作業でも過剰記述にしない。最低限、前提と結果が分かればよい。

```md
Given 対象の画面 / API / データが分かっている
When worker が修正を実装する
Then 期待する結果が再現できる
And 必要なら確認観点や非対象を書く
```
...
既存の Story / Task に紐づかない直接依頼は、必要に応じて単発 Task として `issue_task` で記録する。
